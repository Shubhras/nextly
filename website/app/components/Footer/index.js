   ...
   return (
      <div className=" py-10">
         <div className=" container-custom grid grid-cols-12">
            <div className="md:col-span-3 order-12  md:order-1  sm:py-0 py-5 col-span-12 !">
               <Image loader={({ src }) => src} src={`${IMG_URL + settings.image}`} className="invert w-32 " width="128" height="31" alt="Logo" />
               <h4 className=" text-lg mt-2">{settings.company}</h4>
               ...
               <li key={val.title} className="mt-2 text-left">
                  {!val.children ? (
                     <Link href={val.link ? val.link : "/content/" + val.seo}>
                        <a className="text-2xl">{val.title}</a>
                     </Link>
                  ) : (
                     <>
                        ...
                        <ul key={val.title}>
                           {val?.children.map((val2) => (
                              <li key={val2.title} className=" f13 my-1 sm:my-4  ">
                                 {!val2.children ? (
                                    <Link
                                       href={
            ...
            <div className="  col-span-12 text-center order-10  ">
               <div className="  grid grid-cols-2 md:flex items-center flex-row justify-around   md:grid-cols-3 lg:grid-cols-6">
                  {settings.address
                     ? settings.address.map((val) => (
                        <div key={val.name}>
                           <h5 className="text-xl mt-2">{val.name}</h5>
